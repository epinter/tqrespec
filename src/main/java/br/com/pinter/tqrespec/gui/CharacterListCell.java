...
public class CharacterListCell extends ListCell<PlayerCharacterFile> {
    ...
    public CharacterListCell() {
        ...
        HBox containerPoints = new HBox(characterType);
    ...
    public void updateItem(PlayerCharacterFile character, boolean empty) {
        ...
        if (empty) {
            ...
            characterType.setText(null);
            String type = ResourceHelper.getMessage("characters.store.USER");
            if (character.getLocation() == SaveLocation.MAIN) {
                characterType.setText(null);
                setTooltip(null);
            } else {
                characterType.setText(String.format("[%s]", type).toUpperCase(Locale.ROOT));
                setTooltip(new UIUtils().simpleTooltip(type));
            }
