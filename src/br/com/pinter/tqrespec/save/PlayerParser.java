...
public class PlayerParser {
    ...
    private PlayerParser(String playerName, boolean customQuest) throws IOException, IncompatibleSavegameException {
    ...
    private void loadPlayerChr(boolean customQuest) {
...
//            }
    ...
    private String readString(ByteBuffer byteBuffer, boolean utf16le) {
        ...
        try {
//            if(DBG) Util.log(String.format("Buffer: '%s'",byteBuffer));
            int len = byteBuffer.getInt();
            if (len <= 0) {
                return null;
            }
//            if(DBG) Util.log(String.format("Buffer: '%s', len:%d",byteBuffer,len));
            if (utf16le) {
                len *= 2;
            }
...
//            if(DBG) Util.log(String.format("len=%s; str=%s",len,new String(buf,"UTF-8")));
